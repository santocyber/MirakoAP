...
expand_storage.sh
sudo tee /root/expand_storage.sh >/dev/null <<'EOF'
#!/bin/bash

# Script para expandir a partição raiz para usar todo o cartão SD
echo "=== Iniciando expansão do storage ==="

# Dispositivo e partição
DEVICE="/dev/mmcblk1"
PARTITION="${DEVICE}p1"

# Verificar se o parted está instalado
...
# Expandir a partição usando parted não-interativo
echo "Expandindo a partição..."
parted -s $DEVICE resizepart 1 100%

# Atualizar a tabela de partições
echo "Atualizando tabela de partições..."
partprobe $DEVICE

# Esperar um pouco para o kernel detectar as mudanças
sleep 3
...
chmod +x expand_storage.sh

sudo ./expand_storage.sh
