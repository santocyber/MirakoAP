    ...
    with CFG_LOCK:
        ...
        if "phones" in updates:
            ...
            if isinstance(ph, str):
                ...
                evo["phones"] = [str(p).strip() for p in ph if p.strip()]
...
function addStream(dev){
  ...
  controls.innerHTML = `
    ...
    <!-- >>> NOVO: Ouvir áudio -->
    <label><input type="checkbox" class="audioListen"> Ouvir áudio</label>
  ...
  // Player de áudio (inicialmente oculto)
  ...
  // Lógica do "Ouvir áudio"
  const audioCb  = controls.querySelector('.audioListen');
  const micSelEl = controls.querySelector('.micSel');

  function startAudio(){
    const mic = (micSelEl?.value || 'default').trim();
    const src = url('audio_live?dev='+encodeURIComponent(dev)+'&mic='+encodeURIComponent(mic));
    audioEl.src = src;
    audioEl.style.display = 'block';
    audioEl.play().catch(err=>{
      console.warn('Falha ao iniciar áudio (autoplay pode exigir interação):', err);
    });
  }
  function stopAudio(){
  ...
  audioCb.addEventListener('change', ()=>{
    if (audioCb.checked) startAudio(); else stopAudio();
  });
  micSelEl.addEventListener('change', ()=>{
    if (audioCb.checked) startAudio();
  });
...
# ============ NOVO: Helpers para áudio ao vivo (FFmpeg) ============
def _audio_backends_for_mic(mic: str):
    """
    Decide a ordem de tentativas (backend, source) para este mic.
    Ex.: [("pulse","alsa_input.usb-..."), ("alsa","default"), ("none","")]
    """
    m = (mic or "").strip()
    lower = m.lower()
    out = []
    ...
    if lower.startswith("pulse"):
        src = m.split(":", 1)[1] if ":" in m else "default"
        out.append(("pulse", src))
        out.append(("alsa", "default"))
        out.append(("none",""))
        return out

    if lower.startswith(("hw:", "plughw:", "default", "sysdefault", "front")):
        out.append(("alsa", m))
        out.append(("pulse", "default"))
        out.append(("none",""))
        return out
...
def _spawn_ffmpeg_audio(backend: str, source: str):
    """
    Cria processo FFmpeg que captura áudio e gera MP3 em stdout.
    """
    if backend == "none":
        raise RuntimeError("Nenhum backend de áudio válido para transmitir.")
    if not have_cmd("ffmpeg"):
        raise RuntimeError("ffmpeg não encontrado. Instale o pacote ffmpeg.")

    if backend == "pulse":
        input_args = ["-f", "pulse", "-thread_queue_size", "1024", "-i", source]
    else:
        input_args = ["-f", "alsa", "-thread_queue_size", "1024", "-i", source]
...
# ============ NOVO: Streaming de áudio ao vivo ============
@app.get("/audio_live")
def audio_live():
    dev = request.args.get("dev", "")
    ...
    tries = _audio_backends_for_mic(mic)
    proc = None
    errors = []

    for (backend, source) in tries:
        try:
            if backend == "none":
                break
            proc = _spawn_ffmpeg_audio(backend, source)
            time.sleep(0.4)
            if proc.poll() is None:
                break
            else:
                try:
                    errors.append(proc.stderr.read(200).decode("utf-8", "ignore"))
                except Exception:
                    pass
                proc = None
        except Exception as e:
    ...
    if proc is None or proc.poll() is not None:
        return Response("Falha ao iniciar stream de áudio.\n" + "\n".join(errors[-3:]), status=500)
