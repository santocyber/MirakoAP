...
def get_cam_cfg(dev):
    ...
    with CFG_LOCK:
        ...
        c.setdefault("mic", "default")
        c.setdefault("rotate", 0)  # <<< NOVO: default rotação
        c.setdefault("timelapse", {"enable": False, "interval": 5})
        c.setdefault("motion", {
            "enable": False, "sensitivity": 50, "min_area_pct": 3,
            "action": "photo", "overlay": True, "cooldown": 10, "clip_len": 30
...
<div class="row" style="margin-top:8px;gap:6px;flex-wrap:wrap">
  ...
  <button id="evoTestVideo">Enviar teste (vídeo)</button>   <!-- NOVO -->
...
async function listDevices(){
  ...
  // 2) Renderiza linhas e cria um índice dev -> checkbox
  const CBYDEV = {};
  devs.forEach(dev => {
    ...
    // se já estava marcado no Set, garante a stream
    if (cb.checked) addStream(dev);
  });

  // 3) Se ainda não houver nenhum selecionado, usa os "active" como default
  if (selected.size === 0) {
...
function addStream(dev){
  ...
  const rotVal = parseInt(ccfg.rotate || 0, 10); // <<< NOVO
  ...
  ['.micSel','.rotSel','.tlToggle','.tlInt','.mdToggle','.mdSens','.mdAction','.mdCooldown','.mdClip','.mdOverlay']
  .forEach(sel=>{
...
function gatherCamConfig(dev, controls){
  ...
  const mdC = Math.max(0, parseInt(controls.querySelector('.mdCooldown')?.value,10)||0);
  const mdL = Math.max(5, parseInt(controls.querySelector('.mdClip')?.value,10)||30);
  return {dev, mic, tlE, tlI, mdE, mdS, mdA, mdO, mdC, mdL, rot};
}
  ...
  cams.forEach(cam=>{
    ...
    function section(title, arr, kind){
      ...
      arr.forEach(it=>{
        ...
        // clique no card alterna a seleção (exceto links/checkbox)
...
$('#evoSave').onclick = async ()=>{
  try{
    ...
    // pode não existir, então protegemos:
...
def is_cam_active(dev) -> bool:
    try:
        ...
        # Resolução moderada para sondagem + fps
        ...
        # Algumas câmeras precisam "esquentar": leia alguns frames
        for _ in range(20):  # ~1s com sleeps
            ok, frame = cap.read()
            if ok:
                ok_any = True
        ...
        cap.release()
        # Ativa somente se conseguimos ler E o frame tem conteúdo
...
# ----------------- Recorder e afins (inalterado na lógica principal) -----------------
...
class CameraStream:
    ...
    # usa exatamente o device solicitado (sem remapeamento)
...
def _frame_has_signal(frame) -> bool:
    """
    Retorna True se o frame tiver conteúdo plausível (não nulo/nem completamente uniforme).
    Critério simples: desvio-padrão em escala de cinza acima de um limiar baixo.
    """
    try:
        if frame is None:
            return False
        ...
        return float(std[0][0]) > 2.0  # limiar baixo p/ não rejeitar cena escura
    except Exception:
        return False
...
def api_devices():
    ...
    # Marca como "ativas" apenas as que realmente entregam imagem
    try:
...
def api_config_post():
    ...
    if dev:
        updates = {}
        if "name" in data:
            updates["name"] = str(data["name"]).strip()  # nome amigável da câmera
        if "mic" in data:
            updates["mic"] = data["mic"]

        # NOVO: ângulo de rotação (0/90/180/270)
        if "rotate" in data:
            try:
                rot = int(data["rotate"])
            except Exception:
    ...
    # se não cair em nenhum dos casos acima
...
def api_evo_ping():
    ...
    sender = EVO_SENDER
    s, t = sender._send_text(phone, "ping do MirakoMultiCam ✅")
    return jsonify(ok=(s<300), status=s, resp=t[:500])

@app.get("/api/evo/debug")
def api_evo_debug():
    """Expõe um snapshot da config EVO (sem chave)."""
    with CFG_LOCK:
...
# === EVO: endpoint de teste — tira foto do primeiro device selecionado/primeiro disponível e envia
# --- helper: quais câmeras considerar ativas ---
def _active_devs(prefer_selected=True):
    with CFG_LOCK:
        sel = (CFG.get("ui", {}) or {}).get("selected_devs") or []
        cams = list((CFG.get("cameras") or {}).keys())
    # prioriza as selecionadas; senão, todas que têm config; se nada, tenta /dev/video0
    devs = [str(d) for d in (sel if (prefer_selected and sel) else cams)]
    return devs or ["/dev/video0"]

# --- Evolution: teste em 1..N câmeras (usa Base64 se não houver base) ---
@app.post("/api/evo/test")
def api_evo_test():
    data = request.get_json(silent=True) or {}
    # aceita "dev" (string) ou "devs" (lista). se não vier, usa as ativas da UI
    devs = data.get("devs")
    if not devs:
        d = data.get("dev")
    ...
    for dev in devs:
        dev = str(dev)
        try:
            cam = get_cam(dev)  # abre/pega a stream dessa câmera
            frame = cam.last_frame()
            if frame is None:
            ...
            # dispara envio assíncrono; EvoSender escolhe URL x Base64
...
def api_evo_test_video():
    """
    Grava um clipe de teste por câmera usando o tempo configurado em
    motion.clip_len e envia via Evolution (igual ao movimento).
    Aceita JSON opcional: {"devs": ["/dev/video1", "/dev/video4"], "seconds": 12}
    - se 'seconds' vier, sobrescreve o clip_len da câmera.
    - se 'devs' não vier, usa as selecionadas/ativas (_active_devs).
    A gravação roda em threads para não travar a requisição.
    """
    data = request.get_json(silent=True) or {}
    devs = data.get("devs")
    if not devs:
    ...
    # pode sobrescrever duração via payload
    ...
    def worker(dev, secs):
        try:
            ...
            if out_name:
                ...
                # envia via Evolution (fila assíncrona)
    ...
    for dev in devs:
        try:
            ...
            # lança a gravação em background
